...
exports.execGitCommand = function (command, dir, options, callback) {
    var gitConsoleCommand = getConsoleCommand(dir, command, options).split(' ');
    execFile('git', gitConsoleCommand, callback);
}
...
function getConsoleCommand (dir, command, options) {
    return '--git-dir=' + path.join(dir, getGitDir(options)) + ' --work-tree=' + dir + ' ' + command;
}
